...
BT_DIR = Path('/neelin2020/RGMA_feature_mask/MERGE-IR/{}'.format(year))
...
for mon in range(2,13): 
    ...
    # MERGE-IR hrly output
    files = sorted(list(BT_DIR.glob('merg_{}{}*'.format(year, str(mon).zfill(2)))))
    data_BT = xr.open_mfdataset(files)
    data_BT = coordinates_processors(data_BT)
    data_BT_sub = data_BT.sel(latitude=slice(lat_range[0], lat_range[1]))
    ...
    # interpolate MERGE-IR into ERA5 grid using linear fitting
    data_BT_intp = data_BT_sub.interp(longitude=lon_era5, latitude=lat_era5, method='linear').fillna(0)
    data_BT_intp = data_BT_intp.transpose('time','latitude','longitude')
    ...
    data_FP_merged = xr.merge([data_GPM_intp.resample(time='6H').nearest(),
                               data_BT_intp.resample(time='6H').nearest(),
    ...
    data_FP_merged.to_netcdf(OUT_DIR / 'GPM_feature_merged_{}_final.nc'.format(str(mon).zfill(2)), 
    ...
    print('GPM_feature_merged_{}_final.nc saved...'.format(str(mon).zfill(2)))
